# Warehouse Management System - Microservices Architecture

Розподілений REST застосунок для управління складськими місцями з архітектурою мікросервісів на базі Spring Cloud.

## 🆕 Laboratory Work 5: Web Client Implementation

**NEW!** Додано веб-інтерфейс з Bootstrap 5 та Thymeleaf! Замість консольного клієнта тепер доступний сучасний веб-додаток з інтуїтивним UI.

📖 **Детальна документація**: 
- [WEB_CLIENT_QUICK_START.md](WEB_CLIENT_QUICK_START.md) - Швидкий старт (5 хвилин)
- [WEB_CLIENT_GUIDE.md](WEB_CLIENT_GUIDE.md) - Повна документація веб-клієнта
- [LAB4_SPRING_CLOUD_GUIDE.md](LAB4_SPRING_CLOUD_GUIDE.md) - Spring Cloud архітектура

## Структура проєкту

Проєкт складається з п'яти сервісів та двох клієнтів:

1. **Discovery Service** (порт 8761) - Eureka Server для реєстрації сервісів
2. **API Gateway** (порт 8080) - єдина точка входу для всіх запитів
3. **Place Service** (порт 8081) - управління складськими місцями
4. **Order Service** (порт 8082) - управління замовленнями
5. **Payment Service** (порт 8083) - управління оплатами
6. **Web Client** (порт 8090) - 🌐 **NEW!** веб-інтерфейс з Bootstrap 5
7. **Console Client** - консольний клієнт для взаємодії з системою

## Вимоги

- Java 17 або вище
- Maven 3.6+
- Windows або Linux/MacOS

## Запуск системи

⚠️ **ВАЖЛИВО**: Сервіси повинні запускатися у правильному порядку!

### Windows

1. Запустіть всі мікросервіси (автоматичний запуск у правильному порядку):
   ```batch
   start-all-services.bat
   ```
   
   Скрипт запустить сервіси у такому порядку:
   - Discovery Service (Eureka) - порт 8761
   - API Gateway - порт 8080
   - Place Service - порт 8081
   - Order Service - порт 8082
   - Payment Service - порт 8083

2. Перевірте Eureka Dashboard: http://localhost:8761
   - Переконайтеся, що всі сервіси зареєстровані

3. Після запуску всіх сервісів (зачекайте ~45 секунд), запустіть веб-клієнт:
   ```batch
   start-web-client.bat
   ```
   
   Або консольний клієнт:
   ```batch
   start-console-client.bat
   ```

### Linux/MacOS

1. Надайте права на виконання:
   ```bash
   chmod +x start-all-services.sh start-console-client.sh
   ```

2. Запустіть всі мікросервіси:
   ```bash
   ./start-all-services.sh
   ```

3. Перевірте Eureka Dashboard: http://localhost:8761

4. Після запуску всіх сервісів, запустіть веб-клієнт:
   ```bash
   ./start-web-client.sh
   ```
   
   Або консольний клієнт:
   ```bash
   ./start-console-client.sh
   ```

### Ручний запуск

Альтернативно, можна запустити кожен сервіс окремо (ДОТРИМУЙТЕСЬ ПОРЯДКУ!):

```bash
# 1. Discovery Service (зачекайте ~15 секунд)
cd discovery-service
mvn spring-boot:run

# 2. API Gateway (в новому терміналі, зачекайте ~10 секунд)
cd api-gateway
mvn spring-boot:run

# 3. Place Service (в новому терміналі)
cd place-service
mvn spring-boot:run

# 4. Order Service (в новому терміналі)
cd order-service
mvn spring-boot:run

# 5. Payment Service (в новому терміналі)
cd payment-service
mvn spring-boot:run

# 6. Console Client (в новому терміналі)
cd console-client
mvn spring-boot:run
```

## 🌐 Доступ до сервісів

### 🎯 Web Client (Рекомендовано)
**Веб-інтерфейс**: http://localhost:8090

Сучасний веб-додаток з інтуїтивним інтерфейсом:
- 🏠 Головна сторінка з навігацією
- 📦 Перегляд вільних місць
- ➕ Створення замовлення (форма з валідацією)
- 📋 Список замовлень
- 📊 **Деталі замовлення з комплексною агрегацією** (Замовлення + Платежі)
- 💳 Перегляд платежів
- ⚠️ Обробка помилок з дружніми повідомленнями

### Через API Gateway
Всі клієнти повинні використовувати API Gateway як єдину точку входу:

- **Places API**: http://localhost:8080/api/places
- **Orders API**: http://localhost:8080/api/orders
- **Payments API**: http://localhost:8080/api/payments

### Eureka Dashboard
- **Service Registry**: http://localhost:8761

### Прямий доступ до сервісів (для розробки)
- **Place Service**: http://localhost:8081
- **Order Service**: http://localhost:8082
- **Payment Service**: http://localhost:8083

## OpenAPI Документація

Кожен мікросервіс має автоматично згенеровану OpenAPI документацію:

### Swagger UI (Інтерактивний інтерфейс)
- **Place Service**: http://localhost:8081/swagger-ui.html
- **Order Service**: http://localhost:8082/swagger-ui.html
- **Payment Service**: http://localhost:8083/swagger-ui.html

### OpenAPI JSON контракти
- **Place Service**: http://localhost:8081/api-docs
- **Order Service**: http://localhost:8082/api-docs
- **Payment Service**: http://localhost:8083/api-docs

У Swagger UI ви можете:
- Переглянути всі endpoint'и з детальними описами
- Побачити схеми запитів та відповідей
- Протестувати API безпосередньо в браузері
- Завантажити OpenAPI специфікацію

Детальніше про OpenAPI див. [OPENAPI_DOCUMENTATION.md](OPENAPI_DOCUMENTATION.md)

---

## API Endpoints

### 🔹 Place Service

- `GET /api/places/free` - отримати всі вільні місця
- `GET /api/places/free/type/{type}` - отримати вільні місця за типом
- `GET /api/places/user/{userId}` - отримати місця користувача
- `POST /api/places/give` - видати місця користувачу
- `POST /api/places/free` - звільнити місця

### 🔹 Order Service

- `POST /api/orders` - створити замовлення
- `GET /api/orders/date-range?startDate={date}&endDate={date}` - отримати замовлення за діапазоном дат
- `PUT /api/orders/{orderId}/confirm` - затвердити замовлення
- `PUT /api/orders/{orderId}/cancel` - скасувати замовлення
- `PUT /api/orders/{orderId}/start` - почати замовлення
- `PUT /api/orders/{orderId}/finish` - завершити замовлення

### 🔹 Payment Service

- `POST /api/payments` - створити оплату
- `GET /api/payments` - отримати всі оплати
- `GET /api/payments/user/{userId}` - отримати оплати користувача
- `GET /api/payments/order/{orderId}` - отримати оплати за замовленням

**Примітка**: Всі ці endpoints доступні через API Gateway за адресою `http://localhost:8080`

## Консольний клієнт

Після запуску консольного клієнта доступні наступні команди:

1. Get all free places - отримати всі вільні місця
2. Get places by user ID - отримати місця користувача
3. Create order - створити замовлення
4. Get orders by date range - отримати замовлення за діапазоном дат
5. Confirm order - затвердити замовлення
6. Cancel order - скасувати замовлення
7. Start order - почати замовлення
8. Finish order - завершити замовлення
9. Create payment - створити оплату
10. Get all payments - отримати всі оплати
11. Get payments by user ID - отримати оплати користувача
12. Get payments by order ID - отримати оплати за замовленням

Введіть `help` для показу меню або `exit` для виходу.

## Архітектура

### Spring Cloud Architecture

```
┌─────────────────┐
│ Console Client  │
└────────┬────────┘
         │ All requests via Gateway
         ▼
┌─────────────────────────────────────┐
│      API Gateway (Port 8080)        │
│  - Dynamic Routing                  │
│  - Load Balancing                   │
│  - Circuit Breaker                  │
└────────┬────────────────────────────┘
         │ Service Discovery
         ▼
┌─────────────────────────────────────┐
│  Discovery Service (Port 8761)      │
│  Netflix Eureka Server              │
└────────┬────────────────────────────┘
         │ Service Registration
         ▼
┌────────┴────────────────────────────┐
│                                     │
▼                 ▼                   ▼
┌──────────┐  ┌──────────┐  ┌──────────┐
│  Place   │  │  Order   │  │ Payment  │
│ Service  │  │ Service  │  │ Service  │
│  :8081   │  │  :8082   │  │  :8083   │
└──────────┘  └──────────┘  └──────────┘
```

### Key Patterns Implemented

1. **Service Discovery** - Netflix Eureka для автоматичної реєстрації та виявлення сервісів
2. **API Gateway** - Spring Cloud Gateway як єдина точка входу
3. **Client-Side Load Balancing** - @LoadBalanced RestTemplate з Round Robin
4. **Circuit Breaker** - захист від каскадних відмов

### Тришарова архітектура

Кожен мікросервіс має три шари:

1. **Controller** - обробка HTTP запитів
2. **Service** - бізнес-логіка
3. **Repository** - доступ до даних

### DTO (Data Transfer Objects)

- **Міжшарові DTO** - використовуються для передачі даних між шарами всередині сервісу
- **Міжсервісні DTO** - використовуються для комунікації між мікросервісами

### Бази даних

Кожен мікросервіс має свою окрему базу даних (H2 in-memory):

- Place Service: `place_db`
- Order Service: `order_db`
- Payment Service: `payment_db`

### Міжсервісна комунікація

- **Синхронна комунікація** через REST API
- **Service Discovery** через Eureka (замість hardcoded URLs)
- **Client-Side Load Balancing** для розподілу навантаження
- **Fail-fast** для критичних операцій (Place Service)
- **Fail-silent** для некритичних операцій (Payment Service при перевірці оплат)

### JSON Schema Validation

Використовуються JSON схеми для валідації міжсервісних контрактів:
- `order-service/src/main/resources/schemas/place-service-dto-schema.json`
- `order-service/src/main/resources/schemas/payment-service-dto-schema.json`

## Розподілені транзакції

При невдалому виклику між сервісами:

1. **Order Service → Place Service**: Використовується fail-fast підхід. Якщо Place Service недоступний, операція не виконується і повертається помилка.

2. **Order Service → Payment Service**: Використовується fail-silent підхід при перевірці оплат. Якщо Payment Service недоступний, система продовжує роботу без блокування.

3. **Компенсаційні транзакції**: При скасуванні або завершенні замовлення, місця автоматично звільняються через виклик Place Service.

## Конфігурація

### Spring Cloud Configuration

**Discovery Service** (`discovery-service/src/main/resources/application.properties`):
```properties
server.port=8761
eureka.client.register-with-eureka=false
eureka.client.fetch-registry=false
```

**API Gateway** (`api-gateway/src/main/resources/application.yml`):
```yaml
spring:
  cloud:
    gateway:
      routes:
        - id: place-service
          uri: lb://place-service
          predicates:
            - Path=/api/places/**
```

**Microservices** (Place, Order, Payment):
```properties
eureka.client.service-url.defaultZone=http://localhost:8761/eureka/
eureka.client.register-with-eureka=true
eureka.client.fetch-registry=true
```

**Order Service** (Client-Side Load Balancing):
```java
@Bean
@LoadBalanced
public RestTemplate restTemplate() {
    return new RestTemplate();
}
```

Налаштування кожного сервісу знаходяться у файлах `application.properties` або `application.yml`:
- Порти серверів
- Eureka configuration
- Налаштування баз даних

## Тестування

Після запуску всіх сервісів, можна протестувати систему через консольний клієнт або напряму через HTTP запити (наприклад, за допомогою Postman або curl).

## 🚀 Демонстрація Load Balancing

Для демонстрації Round Robin балансування навантаження:

```bash
# Запустіть кілька інстансів Place Service
cd place-service
mvn spring-boot:run  # Instance 1 (port 8081)

# В іншому терміналі
cd place-service
mvn spring-boot:run -Dspring-boot.run.arguments=--server.port=8091  # Instance 2

# В іншому терміналі
cd place-service
mvn spring-boot:run -Dspring-boot.run.arguments=--server.port=8092  # Instance 3
```

Перевірте Eureka Dashboard - всі три інстанси будуть зареєстровані. Запити від Order Service автоматично розподіляться між ними.

## 📚 Додаткова документація

### Web Client (Lab 5)
- **Quick Start Guide**: [WEB_CLIENT_QUICK_START.md](WEB_CLIENT_QUICK_START.md) - швидкий старт за 5 хвилин
- **Implementation Guide**: [WEB_CLIENT_GUIDE.md](WEB_CLIENT_GUIDE.md) - повна документація веб-клієнта
- **Architecture**: [WEB_CLIENT_ARCHITECTURE.md](WEB_CLIENT_ARCHITECTURE.md) - архітектура та діаграми
- **Testing Checklist**: [WEB_CLIENT_TESTING_CHECKLIST.md](WEB_CLIENT_TESTING_CHECKLIST.md) - чеклист тестування
- **Summary**: [WEB_CLIENT_IMPLEMENTATION_SUMMARY.md](WEB_CLIENT_IMPLEMENTATION_SUMMARY.md) - підсумок реалізації

### Microservices (Lab 4)
- **Spring Cloud Guide**: [LAB4_SPRING_CLOUD_GUIDE.md](LAB4_SPRING_CLOUD_GUIDE.md) - детальна документація про Spring Cloud
- **OpenAPI Documentation**: [OPENAPI_DOCUMENTATION.md](OPENAPI_DOCUMENTATION.md) - документація про REST API
- **API Test Requests**: [API_TEST_REQUESTS.md](API_TEST_REQUESTS.md) - приклади HTTP запитів

## Примітки

- Всі сервіси використовують H2 in-memory бази даних, тому дані не зберігаються між перезапусками
- Для production використання необхідно замінити H2 на повноцінну БД (PostgreSQL, MySQL тощо)
- Консольний клієнт використовує Spring Boot для dependency injection, але не потребує власної бази даних
- **ВАЖЛИВО**: Завжди запускайте Discovery Service першим, інакше інші сервіси не зможуть зареєструватися

## Технології

- **Java 17**
- **Spring Boot 3.1.5**
- **Spring Cloud 2022.0.4**
- **Netflix Eureka** - Service Discovery
- **Spring Cloud Gateway** - API Gateway
- **Spring Cloud LoadBalancer** - Client-Side Load Balancing
- **H2 Database** - In-Memory Database
- **SpringDoc OpenAPI** - API Documentation

